...
export class Character {
  ...
  attackCharacter(target: Character) {
    ...
    this.characterType.targetDistance(this.position, this.characterType.distanceRange, target.position)
  ...
  heal(healed: Character) {
    ...
    if (healed != this) {
      if (healed.faction.length <= 0) {
        ...
        if (!isAllies)
          throw new Error("Characters isn't Allies");
      }
    }
  ...
  private maxAndMinHealth(value: number) {
    ...
    if (value > ENV_HEALTH_MAX) finalHealh = ENV_HEALTH_MAX
